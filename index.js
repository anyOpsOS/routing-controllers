...
function createExecutor(driver, options) {
    ...
    if (options && options.controllers && options.controllers.length) {
        controllerClasses = options.controllers.filter(function (controller) {
            return controller instanceof Function;
        });
        var controllerDirs = options.controllers.filter(function (controller) {
    ...
    if (options && options.middlewares && options.middlewares.length) {
        middlewareClasses = options.middlewares.filter(function (controller) {
            return controller instanceof Function;
        });
        var middlewareDirs = options.middlewares.filter(function (controller) {
            return typeof controller === "string";
        });
    ...
    if (options && options.interceptors && options.interceptors.length) {
        interceptorClasses = options.interceptors.filter(function (controller) {
            return controller instanceof Function;
        });
        var interceptorDirs = options.interceptors.filter(function (controller) {
            return typeof controller === "string";
        });
